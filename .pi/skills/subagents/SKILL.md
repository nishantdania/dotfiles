...
Choose and retain one short tmux session name for this conversation, such as `pi-auth`. Choose a distinct short window name and give the subagent a complete, self-contained task:

```bash
scripts/subagent-tmux spawn pi-auth research 'Investigate the auth flow and report concrete findings; do not modify files.'
```
...
```bash
scripts/subagent-tmux list pi-auth
scripts/subagent-tmux capture pi-auth research
scripts/subagent-tmux send pi-auth research 'Also check token refresh handling.'
scripts/subagent-tmux attach pi-auth
```
